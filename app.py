...
@app.route('/salary_slip', methods=['GET'])
def generate_salary_slip():
    employee_id = request.args.get('employee_id')
    month_year = request.args.get('month_year')
    employee = Employee.query.filter_by(employee_id=employee_id).first()
    if employee:
        html_content = render_template('salary_slip.html', employee=employee, month=month_year)
        pdfkit.from_string(html_content, f"salary_slip_{employee_id}.pdf", configuration=config)
        return f"Salary slip generated successfully! <a href='salary_slip_{employee_id}.pdf'>Download PDF</a>"
    else:
...
@app.route('/salary_slip/<employee_id>')
def generate_salary_slip(employee_id):
    # Fetch the employee from the database based on the employee_id
    employee = Employee.query.filter_by(employee_id=employee_id).first()
    if employee:
        html_content = render_template('salary_slip.html', employee=employee, month="March/2024")
        imgkit.from_string(html_content, f"salary_slip_{employee_id}.png", config=config)
        return html_content
        # return f"Salary slip generated successfully! <a href='salary_slip_{employee_id}.png'>Download Image</a>"
    else:
        return "Employee not found"
